...
export function Environment(name: string): BaseURL {
    return `https://${name}-todo-4m32.encr.app`
}
...
export namespace tasks {
    ...
    completed?: boolean
         ...
         * Delete a task
         */
        public async deleteTask(id: string): Promise<types.TaskResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("DELETE", `/api/tasks/${encodeURIComponent(id)}`)
            return await resp.json() as types.TaskResponse
        }
         ...
         * Get a single task
         */
        public async getTask(id: string): Promise<types.TaskResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/api/tasks/${encodeURIComponent(id)}`)
            return await resp.json() as types.TaskResponse
        }
    ...
    completed?: boolean
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("PUT", `/api/tasks/${encodeURIComponent(id)}`, JSON.stringify(params))
            return await resp.json() as types.TaskResponse
        }
    }
}
...
class BaseClient {
    ...
    constructor(baseURL: string, options: ClientOptions) {
        ...
        if (!BROWSER) {
            this.headers["User-Agent"] = "todo-4m32-Generated-TS-Client (Encore/v1.48.11)";
        }
